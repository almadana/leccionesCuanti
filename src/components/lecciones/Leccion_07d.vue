<template>
  <article class="lesson">
    ...
    <section class="lesson_body">
      <p>Ahora también podrás editar el tamaño de las muestras. ¿Qué pasa con el estadístico cuando el tamaño aumenta? ¿Por qué?</p>
      ...
      <div :class="`population_controls p_range_${population1.p_range(population2)}`">
        <div class="sliders">
          <p>
          Media ({{population1.mean}}) <input type="range" :min="5" :max="35" :step="0.1" :value="population1.mean" @input="update_mean1($event.target.value)">
          </p>
          <p>
          Desvío estándar ({{population1.stdev}}) <input type="range" :min="1" :max="20" :step="0.1" :value="population1.stdev" @input="update_stdev($event.target.value)">
          </p>
          <p>
          Tamaño de la muestra: ({{population1.size}}) <input type="range" :min="1" :max="250" :step="1" :value="population1.size" @input="update_population_size($event.target.value)">
          <p>
          Media ({{population2.mean}}) <input type="range" :min="5" :max="35" :step="0.1" :value="population2.mean" @input="update_mean2($event.target.value)">
          </p>
        </div>
...
var population1 = new Population(population_size, population1_setup.mean, population1_setup.stdev, 5.0, 35.0);
var population2 = new Population(population_size, population2_setup.mean, population2_setup.stdev, 5.0, 35.0);
...
//      current_histo.reset();
    ...
    update_mean1: function(value) {
      window.console.log(value, parseFloat(value));
    ...
    update_stdev: function(value) {
      population1.rescale(population1.mean, value);
      population2.rescale(population2.mean, value);
      histo1.reset();
      histo1.add(population1.members.map(single=>single.x));
      histo2.reset();
      histo2.add(population2.members.map(single=>single.x));
    },
    update_population_size: function(value) {
      var new_size = parseInt(value);
      population1.rescale(null, null, new_size);
      population2.rescale(null, null, new_size);
      histo1.reset();
      histo1.add(population1.members.map(single=>single.x));
      histo2.reset();
      histo2.add(population2.members.map(single=>single.x));
    ...
    sample: function() {
      ...
      this.current_sample = selection.map(item=>item.happiness*30+5);
