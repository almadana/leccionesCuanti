...
var questions =  shuffle([
  new Question({ text: ()=>`¿Qué porcentaje de las mujeres opinan que el nivel de conflicto es fuerte?.`,
    type: "fill",
    answer: () => `${ groups[1].segments[1].percent(groups[1].segments[1].group.total(),1) }`,
    response: 0,
    feedback: {
      right: "Correcto!",
      wrong: "Incorrecto :("
  ...
  new Question({ text: ()=>`¿Qué porcentaje de los hombres opina que el nivel de conflicto es débil?`,
    type: "fill",
    answer: () => `${ groups[0].segments[2].percent(groups[0].segments[2].group.total(),1) }`,
    feedback: {
      right: "Correcto!",
      wrong: "Incorrecto :("
    },
  ...
  new Question({ text: ()=>`¿Qué porcentaje de las personas que opinan que el nivel de conflicto es fuerte es mujer?`,
    type: "fill",
    answer: () => `${ cortes[1].segments[1].percent(cortes[1].segments[1].corte.total(),1) }`,
    feedback: {
      right: "Correcto!",
      wrong: "Incorrecto :("
    },
  ...
  new Question({ text: ()=>`Entre los que opinan que el conflicto no existe, hay más mujeres. ¿Verdadero o falso?.`,
    ...
    validate: function(option) {
      if (
        option.value=="true" && cortes[3].segments[1].value>cortes[3].segments[0].value
        ||
        option.value=="false" && cortes[3].segments[1].value<=cortes[3].segments[0].value
      ) {
        this.status = "right";
      } else {
        this.status = "wrong";
